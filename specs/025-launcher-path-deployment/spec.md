...
**Independent Test**: Can be fully tested by deploying the system, creating a managed local `pi` or `acp` agent, and confirming that its runtime is provisioned through the launcher-managed isolated runtime path rather than directly by the backend process.

**Acceptance Scenarios**:

1. **Given** a fresh deployment using the current single-host container deployment model, **When** a managed local `pi` agent is started, **Then** the system provisions an isolated runtime container through the launcher path and the backend connects to it through the returned session endpoint.
2. **Given** a fresh deployment using the current single-host container deployment model, **When** a managed local `acp` agent is started, **Then** the system provisions an isolated runtime container through the launcher path and the backend connects to it through the returned session endpoint.
3. **Given** a managed local agent runtime is active, **When** another managed local agent is started, **Then** the second runtime is isolated from the first and does not share the first agent’s writable worktree or raw credentials.
...
- **YAGNI Check**: No unrelated runtime families, orchestration platforms, or UI redesigns are added in this phase. The feature is limited to closing the deployment gap for existing managed local `pi` and `acp` agents.
...
- **FR-003**: The system MUST support launcher-managed isolated runtimes for managed local agents in the `pi` and `acp` runtime families in phase 1.
- **FR-004**: The system MUST define a single runtime lifecycle for launcher-managed agents covering provision, healthy ready state, dispatch availability, restart, teardown, and recovery after backend restart.
- **FR-005**: The system MUST require the backend to authenticate to the launcher before requesting runtime provisioning, restart, or teardown actions.
- **FR-006**: The system MUST define that only brokered or environment-scoped runtime credentials may enter isolated runtime containers.
- **FR-007**: The system MUST prohibit raw provider secrets from being written into agent worktree files as part of the isolated runtime path.
- **FR-008**: The system MUST define that the backend remains responsible for repository ownership and worktree creation for managed local agents.
- **FR-009**: The system MUST define that each isolated runtime container mounts only its assigned writable worktree and any explicitly required runtime scratch paths.
- **FR-010**: The system MUST prohibit isolated runtime containers from mounting backend source, backend secret files, or direct database credentials.
- **FR-011**: The system MUST define network isolation expectations for isolated runtime containers, including the allowed egress model and the expected behavior when disallowed access is attempted.
- **FR-012**: The system MUST define the backend-to-launcher interaction needed to obtain a usable session endpoint that fits the existing harness dispatch flow.
- **FR-013**: The system MUST define how health is determined for launcher-managed runtimes and how unhealthy runtimes are surfaced to operators.
- **FR-014**: The system MUST define restart behavior for launcher-managed runtimes, including when the system reattaches, reprovisions, or marks a runtime unavailable.
- **FR-015**: The system MUST define teardown behavior so disabling or deleting a managed local agent removes launcher-managed runtime resources and clears stale runtime status.
- **FR-016**: The system MUST define the deployment changes required to run the launcher path in the current single-host container deployment model, including the launcher service, runtime image expectations, and required environment configuration.
- **FR-017**: The system MUST state whether phase 1 remains opt-in or becomes the default deployment path for managed local agents, and MUST justify that choice against rollout risk.
- **FR-018**: The system MUST define a migration path from current backend-managed local runtimes to launcher-managed isolated runtimes without requiring redesign of unrelated PrimeLoop subsystems.
- **FR-019**: The system MUST define rollout guidance that includes operator validation steps and a documented rollback path if launcher-backed isolation fails in deployment.
- **FR-020**: The system MUST constrain phase 1 to launcher service deployment, current deployment wiring, ACP/PI provisioning through the launcher path, session endpoint integration with the existing harness flow, health/restart/cleanup behavior, and documentation updates.
- **FR-021**: The system MUST explicitly identify non-goals for phase 1 so cluster scheduling, unrelated runtime families, and broader PrimeLoop redesign are excluded from this feature.
...
- **Isolated Runtime Container**: The execution environment for one managed local agent, with its own runtime process, constrained mounts, network policy, and health state.
- **Managed Local Agent**: An agent in the supported `pi` or `acp` runtime families that should execute through the launcher-managed isolated runtime path.
- **Assigned Worktree**: The writable project subtree owned by one managed local agent and mounted into only that agent’s isolated runtime container.
...
- **SC-001**: In a fresh deployment, operators can start a managed local `pi` agent and a managed local `acp` agent and verify that both run through isolated launcher-managed runtimes instead of the legacy backend-managed runtime path.
- **SC-002**: For 100% of managed local agents created in the isolated phase-1 path, each runtime is provisioned with only its assigned writable worktree and no direct backend source or database secret mount.
...
- **SC-005**: Operators can complete the documented migration from the current backend-managed local runtime path to the isolated launcher path using the provided deployment and validation guidance without requiring manual code changes.
...
- Existing durable agent records, worktree-based task flow, and harness dispatch concepts remain in place; this feature changes where managed local runtimes execute, not how delegations are modeled.
- Backend-owned worktree creation remains the safer default for phase 1 than moving repository ownership into the isolated runtime containers.
...
- Phase 1 only needs to close the deployment gap for managed local `pi` and `acp` agents; other runtime families can remain on their existing paths until explicitly specified later.
