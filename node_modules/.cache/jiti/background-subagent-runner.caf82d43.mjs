...
var _settings = await jitiImport("../../shared/settings.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
onChildEvent)
{
  return new Promise((resolve) => {
    ...
    const appendChildEvent = (event) => {
      ...
      (0, _artifacts.appendJsonl)(childEventContext.eventsPath, JSON.stringify({
        ...event,
    ...
    registerInterrupt?.(() => {
      ...
      setTimeout(() => {
        if (!settled) (0, _postExitStdioGuard.trySignalChild)(child, "SIGTERM");
      }, 1000).unref?.();
    });
    ...
    child.on("close", (exitCode, signal) => {
      ...
      outputStream.end();
      const finalOutput = (0, _utils.getFinalOutput)(messages) || rawStdoutLines.join("\n").trim();
...

{
  ...
  for (let index = 0; index < candidates.length; index++) {
    ...
    const { args, env, tempDir } = (0, _piArgs.buildPiArgs)({
    ...
    const hiddenError = run.exitCode === 0 && !run.error ? (0, _utils.detectSubagentError)(run.messages) : null;
    ...
    if (!(0, _modelFallback.isRetryableModelFailure)(error) || index === candidates.length - 1) break;
    attemptNotes.push((0, _modelFallback.formatModelAttemptNote)(attempt, candidates[index + 1]));
  }

  const rawOutput = finalResult?.finalOutput ?? "";
  const resolvedOutput = step.outputPath && finalResult?.exitCode === 0 ?
  (0, _singleOutput.resolveSingleOutput)(step.outputPath, rawOutput, finalOutputSnapshot) :
...

{
  ...
  (0, _artifacts.appendJsonl)(input.eventsPath, JSON.stringify({
...
async function runSubagent(config) {
  ...
  const mutatingFailureStates = flatSteps.map(() => (0, _longRunningGuard.createMutatingFailureState)());
  ...
  const appendControlEvent = (event) => {
    ...
    (0, _artifacts.appendJsonl)(eventsPath, JSON.stringify({
  ...
  const updateStepFromChildEvent = (flatIndex, event) => {
    ...
    if (event.type === "tool_execution_start" && event.toolName) {
      ...
      appendRecentStepOutput(step, (0, _utils.extractTextFromContent)(event.message.content).split("\n").slice(-10));
  ...
  const updateRunnerActivityState = (now) => {
    ...
    statusPayload.lastUpdate = now;
    if (changed) (0, _atomicJson.writeAtomicJson)(statusPath, statusPayload);
    return changed;
  };
