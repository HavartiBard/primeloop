...
export function Runs() {
  const { data = [], isError } = useQuery({ queryKey: ['runs'], queryFn: () => fetchEvents({ agent: 'langgraph', limit: 50 }) })
  const runs = data.filter((e) => e.type === 'run.started' || e.type === 'run.completed')
  return (
    <div className="p-4">
      <h2 className="text-sm text-gray-400 mb-3">Run history</h2>
      {isError && <p className="text-red-400 text-sm">Failed to load run history.</p>}
      {runs.map((e) => (
        <div key={e.id} className="bg-gray-900 rounded px-3 py-2 mb-2 text-xs font-mono text-gray-300">
          {e.type} — {typeof e.payload['run_id'] === 'string' ? e.payload['run_id'] : 'unknown'} — {new Date(e.created_at).toLocaleString()}
        </div>
      ))}
