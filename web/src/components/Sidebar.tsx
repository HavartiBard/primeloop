...
import { Bot, ChevronDown, ChevronRight, PanelLeftClose, PanelLeftOpen } from 'lucide-react'
...
export function Sidebar({ items, primeItems, primeName, current, onNavigate, theme, onToggleTheme, primeConfigOpen = false }: Props) {
  ...
  return (
    <aside className={`hidden shrink-0 border-r border-[var(--border-soft)] bg-[var(--sidebar-bg)] px-2 py-3 transition-[width] duration-200 lg:flex lg:flex-col lg:gap-0.5 ${collapsed ? 'w-[4.5rem]' : 'w-48'}`}>
      {/* Logo */}
      <div className={`mb-2 flex items-center rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] px-3 py-2.5 ${collapsed ? 'justify-center' : 'gap-2.5'}`}>
        ...
        {!collapsed && <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--muted)]">PrimeLoop</span>}
      </div>
      ...
      <button
        ...
        className="mb-2 flex w-full items-center justify-center rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] px-3 py-2 text-[var(--muted)] transition hover:bg-[var(--panel-strong)] hover:text-[var(--text)]"
      >
      ...
      {/* Main nav */}
      {items.map((item) => (
        <NavButton key={item.href} item={item} current={current} onNavigate={onNavigate} collapsed={collapsed} />
      ))}
      ...
      {!collapsed ? (
        ...
        <>
          <button
            ...
            onClick={() => setPrimeExpanded((v) => !v)}
            title={primeName}
            className="flex w-full items-center justify-center rounded-xl px-2 py-2 text-[var(--accent-strong)] transition hover:bg-[var(--panel-subtle)]"
          >
      ...
      <div className="mt-auto pt-2">
        ...
        <button
          ...
          title="Cycle themes"
          className={`flex w-full items-center rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] px-3 py-2 text-xs font-semibold text-[var(--text)] transition hover:bg-[var(--panel-strong)] ${collapsed ? 'justify-center' : 'gap-2.5'}`}
        >
          <span className="text-base leading-none">🎨</span>
          {!collapsed && <span className="text-[var(--muted)] capitalize">{theme}</span>}
        </button>
      </div>
    </aside>
  )
...
function NavButton({
  ...
  return (
    <button
      ...
      title={item.label}
      disabled={disabled}
      className={`relative flex w-full items-center rounded-xl text-left transition ${
        collapsed
          ? 'justify-center px-2 py-2 text-sm'
          : sub ? 'gap-2.5 px-2.5 py-1.5 text-xs' : 'gap-2.5 px-3 py-2 text-sm'
      } ${
        active
